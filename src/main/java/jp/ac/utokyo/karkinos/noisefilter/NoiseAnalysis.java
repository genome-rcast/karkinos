...
public class NoiseAnalysis {
	...
	public void analysisNoiseRegion(DataSet dataset, int ploidy) {
		...
		binlist = getBorderDepth(dataset.getSnvlist(), tumorContentsRatio, binsize);
		...
		for (SNVHolder snv : dataset.getSnvlist()) {
			...
			if (check && snv.getFilterResult() != null
					&& (snv.getFilterResult().isPassFilter() || (snv.getFilterResult().getPassFilterFlg()
	...
	public boolean reject(SNVHolder snv, float tumorContentsRatio, boolean indel) {
		...
		if ((oxoGCand || ffpeCand) && onedirec) {
			reject = afm._reject(depth_c, adjusttedAF);
			if (reject) {
				return true;
			}
		}

		reject = afm.reject(depth_c, adjusttedAF, highErrorSample);
		if (reject) {
			return true;
		}
